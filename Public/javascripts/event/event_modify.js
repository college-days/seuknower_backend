		...
		if($("#title").val().replace(/[ ]/g,"") && $("#startdate").val().replace(/[ ]/g,"") && $("#enddate").val().replace(/[ ]/g,"") && $("#location").val().replace(/[ ]/g,"") && window.editor.html().replace(/[ ]/g,"") && $("#organizer").val().replace(/[ ]/g,"")){
			if(iconWidth && iconHeight){
				...
				$.post('/event/thumb_poster', {
					...
					$("#intro").val(window.editor.html().replace(/\s(style|class).[^<\s]*"\B/g,""));
					$('#event').submit();
				},'json');
			}else{
				if(!uploading){
...
function setPreviousLastTime(){
	var previousStartLastTime = $("#previousstartlasttime").text();
	var previousEndLastTime = $("#previousendlasttime").text();
	console.log(previousStartLastTime);
	$("#startlasttime").find("option[value='"+previousStartLastTime+"']").attr("selected", true);
	$("#endlasttime").find("option[value='"+previousEndLastTime+"']").attr("selected", true);
}
